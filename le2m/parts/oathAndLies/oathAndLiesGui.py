...
from oathAndLiesGuiSrc import OL_widgains, OL_widDe
...
class WDe(QtGui.QWidget):
    def __init__(self, automatique=False, parent=None, autotime=1000):
        super(WDe, self).__init__(parent)

        self.ui = OL_widDe.Ui_Form()
        self.ui.setupUi(self)

        self.ui.pushButton_de.clicked.connect(self._draw)
        self.ui.label_de_text.setText(u"Résultat du dé")
        self.ui.label_de.setText(u"?")
        ...
        if automatique:
            self._timer = QtCore.QTimer()
            self._timer.timeout.connect(self.ui.pushButton_de.click)
            self._timer.start(autotime)

    def _draw(self):
        self._value = random.randint(1, 6)
        self.ui.label_de.setText(str(self._value))
...
class DDecisionA(QtGui.QDialog):
    def __init__(self, defered, automatique, parent):
        ...
        self._widgains.ui.pushButton_ok.clicked.connect(
            lambda _: self._widde.setEnabled(True))
        ...
        self._widde = WDe(automatique=self._automatique, parent=self,
                          autotime=2000)
        self._widde.ui.pushButton_de.clicked.connect(
                lambda _: self._widmessage.setEnabled(True))
        layout.addWidget(self._widde)
        self._widde.setEnabled(False)
    ...
    def _accept(self):
        ...
        if not self._automatique:
            ...
            if conf != QtGui.QMessageBox.Yes:
                return
        dec = {"dice": self._widde.get_value(),
