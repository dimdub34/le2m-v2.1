...
from CommonPoolResourceTexts import _CPR
import CommonPoolResourcePart  # for sqlalchemy
...
class Serveur(object):
    def __init__(self, le2mserv):
        ...
        # server screen
        actions = OrderedDict()
        actions[_CPR(u"Configure")] = self._configure
        actions[_CPR(u"Display parameters")] = \
            lambda _: self._le2mserv.gestionnaire_graphique. \
            display_information2(
                utiltools.get_module_info(pms), u"Paramètres")
        actions[_CPR(u"Start")] = lambda _: self._demarrer()
        actions[_CPR(u"Display payoffs")] = \
            lambda _: self._le2mserv.gestionnaire_experience.\
            display_payoffs("CommonPoolResource")
        actions[_CPR(u"Show graph")] = self._show_fig
    ...
    def _demarrer(self):
        ...
        confirmation = self._le2mserv.gestionnaire_graphique.\
            question(_CPR(u"Start CommonPoolResource?"))
        if not confirmation:
            return
        ...
        for period in xrange(1 if pms.NOMBRE_PERIODES else 0,
            ...
            self._le2mserv.gestionnaire_graphique.infoserv(
                [None, _CPR(u"Period {}").format(period)])
            self._le2mserv.gestionnaire_graphique.infoclt(
                [None, _CPR(u"Period {}").format(period)], fg="white", bg="gray")
            yield (self._le2mserv.gestionnaire_experience.run_func(
                self._tous, "newperiod", period))
            
            # décision ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            yield(self._le2mserv.gestionnaire_experience.run_step(
                _CPR(u"Decision"), self._tous, "display_decision"))

            self._le2mserv.gestionnaire_graphique.infoserv(
                _CPR(u"Sum of decisions"))
            ...
            yield(self._le2mserv.gestionnaire_experience.run_step(
                _CPR(u"Summary"), self._tous, "display_summary"))

            # stats period
            self._le2mserv.gestionnaire_graphique.infoserv(
                _CPR(u"Av. for the period"))
            dataperiod = []
            ...
            for c, g in enumerate(self._groups):
                ...
                yval[c].append(df_dataperiod["CPR_decision"].ix[g])
        ...
        self._le2mserv.gestionnaire_graphique.infoserv(
            [None, _CPR(u"Av. for the whole game")])
